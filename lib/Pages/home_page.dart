...
class _HomePageState extends State<HomePage> {
  ...
  // Cache items per category NAME (String)
  ...
  void initState() {
    super.initState();
    // Clear the cache when the widget initializes
    _cachedItems.clear(); 
  ...
  Future<void> _fetchMenuItems(String categoryName) async {
    // Check cache first using category name as key
    if (_cachedItems.containsKey(categoryName)) {
      // Check if widget is still mounted before calling setState
      if (!mounted) return;
      setState(() {
        // Get items from cache using name
        _displayedItems = _cachedItems[categoryName]!;
        _isLoading = false;
        _errorMessage = null;
        _updateCartStatus();
      });
    ...
    setState(() {
      _isLoading = true;
      _errorMessage = null;
      // Keep currently displayed items until new ones load? Or clear?
      // _displayedItems = []; // Optional: Clear immediately
    });
    ...
    try {
      ...
      setState(() {
        ...
        // Cache the fetched items using name as key
  ...
  void _updateItemState(MenuItem item, Function(MenuItem foundItem) updateAction) {
    ...
    print(">>> _updateItemState called for: ${item.id} (${item.name}), Category: ${item.category}");
    if (_cachedItems.containsKey(item.category)) {
      print(">>> Cache contains category: ${item.category}");
      var categoryList = _cachedItems[item.category]!;
      print(">>> Searching for ID ${item.id} in category list of size ${categoryList.length}");
      int itemIndex = categoryList.indexWhere((cachedItem) => cachedItem.id == item.id);
      print(">>> Result of indexWhere: $itemIndex"); // Check if -1 (not found)
      // ---- End Detailed Logging ----
      if (itemIndex != -1) {
        print(">>> Item found at index $itemIndex. Applying update to CACHED item.");
        // Apply update ONLY to the item in the cache
        updateAction(categoryList[itemIndex]);
      } else {
        // Log if item not found in its supposed category cache (shouldn't usually happen)
        print(">>> ERROR: Item ${item.id} (${item.name}) NOT FOUND in cached category ${item.category}");
        // Fallback: Just update the item instance we have (should not be needed now)
        // updateAction(item);
      }
    } else {
      // Log if the item's category isn't even in the cache (indicates a potential issue)
      print(">>> ERROR: Category ${item.category} NOT FOUND in cache for item ${item.id} (${item.name})"); // More prominent error log
      // Fallback: Just update the item instance we have
      updateAction(item);
    }
    _updateCartStatus(); // Update overall cart status after any change
     print(">>> _updateItemState finished. Recalculating cart status."); 
  }
                ...
                child: GestureDetector(
                  ...
                  child: Stack(
                    children: [
                      ...
                      // --- Restore Light Yellow Background Overlay --- 
                      if (item.count > 0) // Show only if count > 0
                          Container( 
                                decoration: BoxDecoration(
                                borderRadius: BorderRadius.vertical(top: Radius.circular(10)),
                                // color: Colors.amber.withOpacity(0.2), // Remove solid color
                                // --- Add Gradient --- 
                                  gradient: LinearGradient(
                                      begin: Alignment.bottomCenter,
                                  end: Alignment.topCenter,
                                      colors: [
                                    Colors.amber.withOpacity(0.4), // Light yellow at the bottom (adjust opacity as needed)
                                    Colors.transparent,          // Fully transparent at the top
                                  ],
                                  stops: [0.0, 0.75] // Start solid yellow, fade out over top 75% (adjust stops as needed)
                                )
                                // --- End Gradient --- 
                            ),
                          ),
